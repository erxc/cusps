...
est_rec_final <- est_rec[,,glm_warning_ind == 0 & 
                           apply(est_rec,3,function(x) sum(x<0)) == 0 & 
                           apply(est_rec,3,function(x) sum(is.na(x))) == 0]
...
write.csv(tau10_est, "MW_results/tau10_est_dml_nn.csv", row.names = F)
write.csv(tau00_est, "MW_results/tau00_est_dml_nn.csv", row.names = F)
write.csv(tau11_est, "MW_results/tau11_est_dml_nn.csv", row.names = F)
