...
for (i in 1:n_rep) {
  ...
  tryCatch(est <- moment_pi_p_mw_est(df), 
           warning = function(w) {
             print(w)
             glm_warning_ind[i] <<- 1
           })
...
write.csv(tau10_est, "MW_results/tau10_est_pi_p.csv", row.names = F)
write.csv(tau00_est, "MW_results/tau00_est_pi_p.csv", row.names = F)
write.csv(tau11_est, "MW_results/tau11_est_pi_p.csv", row.names = F)
