...
for (i in 1:n_rep) {
  ...
  tryCatch(est <- triply_robust_mw_est(df), 
           warning = function(w) {
             print(w)
             glm_warning_ind[i] <<- 1
           })
...
write.csv(tau10_est, "MW_results/tau10_est_tr.csv", row.names = F)
write.csv(tau00_est, "MW_results/tau00_est_tr.csv", row.names = F)
write.csv(tau11_est, "MW_results/tau11_est_tr.csv", row.names = F)
