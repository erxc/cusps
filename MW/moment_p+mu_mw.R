...
moment_p_mu_mw_est <- function(df) {
  ...
  ## different misspecification scenarios
  ### define functions
  tauN10 <- function(ps1_x_i, ps0_x_i, mu11_x_i, mu00_x_i,
                     ps1_x_j, ps0_x_j, mu11_x_j, mu00_x_j, sig) {
    
    g10_oioj <- 0.5*((ps1_x_i-ps0_x_i) * (ps1_x_j-ps0_x_j) * 
                       pnorm((mu11_x_i-mu00_x_j)/(sqrt(2)*sig)) + 
                       (ps1_x_j-ps0_x_j) * (ps1_x_i-ps0_x_i) *
                       pnorm((mu11_x_j-mu00_x_i)/(sqrt(2)*sig)))
    
    return(mean(g10_oioj))
  }
  
  tauN00 <- function(ps1_x_i, mu10_x_i, mu00_x_i, 
                     ps1_x_j, mu10_x_j, mu00_x_j, sig) {
    
    g00_oioj <- 0.5*((1-ps1_x_i) * (1-ps1_x_j) * pnorm((mu10_x_i-mu00_x_j)/(sqrt(2)*sig)) + 
                       (1-ps1_x_j) * (1-ps1_x_i) * pnorm((mu10_x_j-mu00_x_i)/(sqrt(2)*sig)))
    
    return(mean(g00_oioj))
  }
  
  tauN11 <- function(ps0_x_i, mu11_x_i, mu01_x_i, 
                     ps0_x_j, mu11_x_j, mu01_x_j, sig) {
    
    g11_oioj <- 0.5*(ps0_x_i * ps0_x_j * pnorm((mu11_x_i-mu01_x_j)/(sqrt(2)*sig)) + 
                       ps0_x_j * ps0_x_i * pnorm((mu11_x_j-mu01_x_i)/(sqrt(2)*sig)))
  ...
  ### tp-T, ps-T, oc-T
  tauN10_tpT_psT_ocT <- 
    tauN10(ps1_x_i = dfp$ps1_x, ps0_x_i = dfp$ps0_x, mu11_x_i = dfp$mu11_x, mu00_x_i = dfp$mu00_x,
           ps1_x_j = dfp$ps1_x_2, ps0_x_j = dfp$ps0_x_2,  mu11_x_j = dfp$mu11_x_2, mu00_x_j = dfp$mu00_x_2, 
           sig = sigma)
  
  tauN00_tpT_psT_ocT <- 
    tauN00(ps1_x_i = dfp$ps1_x, mu10_x_i = dfp$mu10_x, mu00_x_i = dfp$mu00_x,
           ps1_x_j = dfp$ps1_x_2, mu10_x_j = dfp$mu10_x_2, mu00_x_j = dfp$mu00_x_2, sig = sigma)
  
  tauN11_tpT_psT_ocT <- 
    tauN11(ps0_x_i = dfp$ps0_x, mu11_x_i = dfp$mu11_x, mu01_x_i = dfp$mu01_x,
           ps0_x_j = dfp$ps0_x_2, mu11_x_j = dfp$mu11_x_2, mu01_x_j = dfp$mu01_x_2, sig = sigma)
  ...
  ### tp-T, ps-F, oc-T
  tauN10_tpT_psF_ocT <- 
    tauN10(ps1_x_i = dfp$ps1_x_m, ps0_x_i = dfp$ps0_x_m, mu11_x_i = dfp$mu11_x, mu00_x_i = dfp$mu00_x,
           ps1_x_j = dfp$ps1_x_m_2, ps0_x_j = dfp$ps0_x_m_2, mu11_x_j = dfp$mu11_x_2, mu00_x_j = dfp$mu00_x_2, 
           sig = sigma)
  
  tauN00_tpT_psF_ocT <- 
    tauN00(ps1_x_i = dfp$ps1_x_m, mu10_x_i = dfp$mu10_x, mu00_x_i = dfp$mu00_x,
           ps1_x_j = dfp$ps1_x_m_2, mu10_x_j = dfp$mu10_x_2, mu00_x_j = dfp$mu00_x_2, sig = sigma)
  
  tauN11_tpT_psF_ocT <- 
    tauN11(ps0_x_i = dfp$ps0_x_m, mu11_x_i = dfp$mu11_x, mu01_x_i = dfp$mu01_x,
           ps0_x_j = dfp$ps0_x_m_2, mu11_x_j = dfp$mu11_x_2, mu01_x_j = dfp$mu01_x_2, sig = sigma)
  
  ### tp-T, ps-T, oc-F
  tauN10_tpT_psT_ocF <- 
    tauN10(ps1_x_i = dfp$ps1_x, ps0_x_i = dfp$ps0_x, mu11_x_i = dfp$mu11_x_m, mu00_x_i = dfp$mu00_x_m,
           ps1_x_j = dfp$ps1_x_2, ps0_x_j = dfp$ps0_x_2, mu11_x_j = dfp$mu11_x_m_2, mu00_x_j = dfp$mu00_x_m_2, 
           sig = sigma_m)
  
  tauN00_tpT_psT_ocF <- 
    tauN00(ps1_x_i = dfp$ps1_x, mu10_x_i = dfp$mu10_x_m, mu00_x_i = dfp$mu00_x_m,
           ps1_x_j = dfp$ps1_x_2, mu10_x_j = dfp$mu10_x_m_2, mu00_x_j = dfp$mu00_x_m_2, sig = sigma_m)
  
  tauN11_tpT_psT_ocF <- 
    tauN11(ps0_x_i = dfp$ps0_x, mu11_x_i = dfp$mu11_x_m, mu01_x_i = dfp$mu01_x_m,
           ps0_x_j = dfp$ps0_x_2, mu11_x_j = dfp$mu11_x_m_2, mu01_x_j = dfp$mu01_x_m_2, sig = sigma_m)
  ...
  ### tp-T, ps-F, oc-F
  tauN10_tpT_psF_ocF <- 
    tauN10(ps1_x_i = dfp$ps1_x_m, ps0_x_i = dfp$ps0_x_m, mu11_x_i = dfp$mu11_x_m, mu00_x_i = dfp$mu00_x_m,
           ps1_x_j = dfp$ps1_x_m_2, ps0_x_j = dfp$ps0_x_m_2, mu11_x_j = dfp$mu11_x_m_2, mu00_x_j = dfp$mu00_x_m_2, 
           sig = sigma_m)
  
  tauN00_tpT_psF_ocF <- 
    tauN00(ps1_x_i = dfp$ps1_x_m, mu10_x_i = dfp$mu10_x_m, mu00_x_i = dfp$mu00_x_m,
           ps1_x_j = dfp$ps1_x_m_2, mu10_x_j = dfp$mu10_x_m_2, mu00_x_j = dfp$mu00_x_m_2, sig = sigma_m)
  
  tauN11_tpT_psF_ocF <- 
    tauN11(ps0_x_i = dfp$ps0_x_m, mu11_x_i = dfp$mu11_x_m, mu01_x_i = dfp$mu01_x_m,
           ps0_x_j = dfp$ps0_x_m_2, mu11_x_j = dfp$mu11_x_m_2, mu01_x_j = dfp$mu01_x_m_2, sig = sigma_m)
